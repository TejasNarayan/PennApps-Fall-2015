...
app.post('/incoming', function(req, res) {
  ...
  if (message == 'y' || message == 'Y' || message == 'Yes' || message == 'yes') {
    ...
    request = new Request("UPDATE messages SET messages.time_replied = GETDATE() FROM users WHERE users.id = messages.sent_to AND messages.time_replied IS NIL AND users.phone = " + from + ";" +
    "UPDATE messages SET messages.reply = YES;", function(err) {
    if (err) {
        console.log(err);}
    });
